...
def test_expire_pending_reservations_expires_all_valid_expired_reservations():
    ...
    try:
        ...
        seat2 = SeatModel(
            venue_id=venue.id, section="Main", row_label="A", seat_number=11
        )
        ...
        reservation2 = ReservationModel(
            ...
            user_id=user.id,
            status=ReservationStatus.PENDING,
            expires_at=datetime.now(ZoneInfo("Europe/Athens")) - timedelta(minutes=2),
        )
    ...
    try:
        ...
        try:
            reservation1 = (
                ...
                .filter(ReservationModel.id == reservation1_id)
                .first()
            )
            reservation2 = (
                check_db.query(ReservationModel)
                .filter(ReservationModel.id == reservation2_id)
                .first()
            )
            saved_event_seat1 = (
            ...
            saved_event_seat2 = (
                check_db.query(EventSeatModel)
                .filter(EventSeatModel.id == event_seat2_id)
                .first()
            )
...
def test_expire_pending_reservations_uses_skip_locked():
    ...
    try:
        ...
        worker1_db.query(ReservationModel).filter(
            ReservationModel.id == reservation1_id,
            ReservationModel.status == ReservationStatus.PENDING,
            ReservationModel.expires_at <= datetime.now(ZoneInfo("Europe/Athens")),
