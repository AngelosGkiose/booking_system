...
def test_create_reservation_holds_available_event_seat():
    ...
    try:
        ...
        db.add(seat1)
        db.flush()
        event1 = EventModel(venue_id=venue.id, name="New Event1", start_time=datetime(2026, 9, 10, 20, 0, tzinfo=ZoneInfo("Europe/Athens")), end_time=datetime(2026, 9, 10, 22, 0, tzinfo=ZoneInfo("Europe/Athens")))
        db.add(event1)
        db.flush()
        event_seat1 = EventSeatModel(seat_id=seat1.id, event_id=event1.id, price=10.0)
        db.add(event_seat1)
        db.flush()
        reservation = create_reservation_service("test-key-create", event_seat1.id, db, user)
...
def test_create_reservation_returns_404_when_event_seat_not_found():
    ...
    try:
        ...
        with pytest.raises(HTTPException) as exc:
            create_reservation_service("test-key-not-found", -1, db, user)
...
def test_create_reservation_returns_409_when_event_seat_is_held():
    ...
    try:
        ...
        db.flush()
        with pytest.raises(HTTPException) as exc:
            create_reservation_service("test-key-held", event_seat1.id, db, user)
...
def test_create_reservation_returns_409_when_event_seat_is_reserved():
    ...
    try:
        ...
        db.flush()
        with pytest.raises(HTTPException) as exc:
            create_reservation_service("test-key-reserved", event_seat1.id, db, user)
        assert exc.value.status_code == 409
    finally:
        db.rollback()
        db.close()
...
def test_create_reservation_rolls_back_on_failure(monkeypatch):
    ...
    try:
        ...
        with pytest.raises(Exception, match="Commit failed"):
            create_reservation_service("test-key-rollback", event_seat_id, db, user)

        check_db = SessionLocal()
        try:
            saved_event_seat = check_db.query(EventSeatModel).filter(EventSeatModel.id == event_seat_id).first()
            reservations = check_db.query(ReservationModel).filter(ReservationModel.event_seat_id == event_seat_id).all()
...
def test_create_reservation_prevents_double_booking():
    ...
    try:
        ...
        user2 = UserModel(email="agg12321@gmail.com", hashed_password="123")
        db.add(user1)
        db.flush()
        user1_id = user1.id
        db.add(user2)
...
def reserve(user_id, event_seat_id, barrier):
    ...
    try:
        ...
        barrier.wait()
        create_reservation_service(f"double-booking-{user_id}", event_seat_id, db, user)
