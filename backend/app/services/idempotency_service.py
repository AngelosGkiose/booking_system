...
from app.repositories.idempotency_repository import (
    create_idempotency_request_repo,
...
def create_idempotency_request(user_id, key, request_hash, db):
    try:
        idempotency_request = create_idempotency_request_repo(
        ...
        db.rollback()
        existing_idempotency_request = get_idempotency_request_by_user_and_key(
            user_id, key, db
        )
        ...
        if existing_idempotency_request.status == IdempotencyRequestEnum.COMPLETED:
            ...
            raise HTTPException(
                status_code=existing_idempotency_request.response_status,
                detail=existing_idempotency_request.error_detail,
            )


def mark_idempotency_completed(
    idempotency_request, reservation_id, response_status, db
):
