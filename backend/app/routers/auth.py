...
from app.services.rate_limit_service import clear_rate_limit, record_rate_limit_attempt, \
...
def login_user(request: Request,form_data = Depends(OAuth2PasswordRequestForm),db:Session=Depends(get_db)):
     ...
     try:
          ...
          if exc.status_code == status.HTTP_401_UNAUTHORIZED:
               ...
               email_remaining,email_ttl,email_blocked=record_rate_limit_attempt(email_key, 5, 300)
               if ip_blocked :
                   raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Too many login requests",headers={"Retry-After": str(ip_ttl), "X-RateLimit-Limit": "5",
                                                "X-RateLimit-Remaining": str(ip_remaining),"X-RateLimit-Reset":str(ip_ttl)})
               elif email_blocked :
                    raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Too many login requests",
                                        headers={"Retry-After": str(email_ttl), "X-RateLimit-Limit": "5",
...
def refresh_access_token(request:Request,response:Response,data:RefreshTokenRequest,db:Session = Depends(get_db)):
     client=request.client
     if client is None:
          client_ip = "unknown"
     ...
     if blocked :
          raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail="Too many refresh requests",
                              headers={"Retry-After": str(ttl), "X-RateLimit-Limit": "20",
                                       "X-RateLimit-Remaining": str(remaining), "X-RateLimit-Reset": str(ttl)})
