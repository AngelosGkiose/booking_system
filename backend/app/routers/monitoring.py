...
@router.get("/outbox",response_model=MonitoringOutboxEvents,status_code=status.HTTP_200_OK)
def monitoring_outbox(current_user: UserModel = Depends(require_admin),db: Session = Depends(get_db)):
