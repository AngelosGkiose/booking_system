...
def get_user_refresh_token(user_id, jti, db):
    return (
        ...
        .filter(RefreshTokenModel.jti == jti, RefreshTokenModel.user_id == user_id)
        .first()
    )
...
def get_user_sessions_repo(session_id, user_id, db):
    return (
        db.query(RefreshTokenModel)
        .filter(
            RefreshTokenModel.user_id == user_id, RefreshTokenModel.id == session_id
        )
        .first()
    )
...
def get_user_all_sessions_repo(user_id, db):
    return (
        db.query(RefreshTokenModel).filter(RefreshTokenModel.user_id == user_id).all()
    )
