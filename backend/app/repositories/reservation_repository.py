...
def get_user_reservations_repo(event_id,reservation_status,skip,date_from,date_to,limit,current_user_id,db):
...
def count_user_reservations_repo(event_id,reservation_status,date_from,date_to,current_user_id,db):
    query = db.query(ReservationModel).join(EventSeatModel,ReservationModel.event_seat_id==EventSeatModel.id).filter(ReservationModel.user_id == current_user_id)
    if event_id is not None:
        query = query.filter(EventSeatModel.event_id == event_id)
    if reservation_status is not None:
